...
RedactorPlugins.glyphicons = {
	init: function()
	{

		// load all glyphs into array (taken from Bootstrap Glyph list)
		...
		var modalHtml = '<section><ul class="redactor_glyphs_box">';
		$.each(glyphicons ,function(i, v) {
			modalHtml += '<li><a href="#" class="redactor_glyphs_link"><span class="glyphicon '+v+'"></span></a></li>';
		});
		modalHtml += '</ul></section>';
		
		var callback = $.proxy(function()
		{
			$('#redactor_modal').find('.redactor_glyphs_link').each($.proxy(function(i, s)
			{
				$(s).click($.proxy(function()
				{
					this.insertGlyph($(s).html());
					return false;
		...
		this.buttonAdd('glyphicons', 'Glyphicon', function(e)
		{
			this.modalInit('Insert Glyphicon', modalHtml, 500, callback);
		});
	},
	insertGlyph: function(html)
	{
